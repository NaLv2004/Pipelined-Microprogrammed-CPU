...
module cu (
    ...
    output o_exec_ready_combined // NEEDS ASSIGNMENT!!
);
wire clk;
...
// ready to fetch the next instruction when either of the sequencing modules are ready
...
// speculative micro-instruction fetch
// fetch the next micro-instruction speculatively
// this process is done only when the normal CU has received an instruction with multiple micro-instructions
// then, the module speculatively fetches the next micro-instruction and checks whether it is in conflict with the current micro-instruction fetched by 
// normal cu. If it is, the speculative fetch is cancelled and the normal fetch is continued. If it is not, the speculative fetch is continued.
module speculative_micro_instruction_fetch ( 
    input clk,
    input rst,
...
wire [7:0] test_b = 8'b0101_0010;
wire [7:0] test_c = 8'b1100_0001;

wire test_ab = |(test_a&test_b); // should be 0
wire test_ac = |(test_a&test_c); // should be 1
...
cu u_cu(
    ...
    .micro_code_out(micro_code_cu_alu)
);
...
u_micro_instr_mem.mem[33] = 16'b0000_0000_0000_0010; // memory write enable (writing MBR to memory) : can be done in parallel
